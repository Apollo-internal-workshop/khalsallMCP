...
export default function Product() {
  ...
  const setVariantOptions = (product) => {
    if (product) {
      const colorOpts = product.variants
      ...
      setSizeOptions(allSizes.map(variant => variant.size).sort((a, b) => sizeOrder.indexOf(a) - sizeOrder.indexOf(b)));
      setInStockSizes(inStockSizeOfColor);
    }
  };

  useEffect(() => {
    setVariantOptions(data.product);
